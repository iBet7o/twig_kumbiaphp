...
class PropertyAccessor
{
    ...
    public function getValue($objectOrArray, $propertyPath)
    {
        ...
        } elseif (!$propertyPath instanceof PropertyPath) {
            throw new InvalidArgumentException($propertyPath, 'debe ser un string o una instancia de PropertyPath');
        }
     ...
     */
    private function &readPropertiesUntil(&$objectOrArray, PropertyPath $propertyPath, $lastIndex, $throwExceptionOnNonexistantIndex = false)
    {
        ...
        for ($i = 0; $i < $lastIndex; ++$i) {
            ...
            if ($isIndex && $isArrayAccess && !isset($objectOrArray[$property])) {
                if ($throwExceptionOnNonexistantIndex) {
                    throw new OutOfRangeException(sprintf('Cannot read property "%s". Available properties are "%s"', $property, print_r(array_keys($objectOrArray), true)));
                }
     ...
     *
     * @throws InvalidArgumentException If the array does not implement \ArrayAccess or it is not an array
     */
    private function &readIndex(&$array, $index)
    {
        if (!$array instanceof ArrayAccess && !is_array($array)) {
            throw new InvalidArgumentException(sprintf('Index "%s" cannot be read from object of type "%s" because it doesn\'t implement \ArrayAccess', $index, get_class($array)));
        }
     ...
     *
     * @throws InvalidArgumentException If the property does not exist or is not
     *                                 public.
     */
    private function &readProperty(&$object, $property)
    {
        ...
        if (!is_object($object)) {
            throw new InvalidArgumentException(sprintf('Cannot read property "%s" from an array. Maybe you should write the property path as "[%s]" instead?', $property, $property));
        }
        ...
        if ($reflClass->hasMethod($getter) && $reflClass->getMethod($getter)->isPublic()) {
            ...
            throw new InvalidArgumentException(sprintf(
                    ...
                    'class "%s".', $property, $getter, $isser, $hasser, $reflClass->name
            ));
        }
    ...
    {
        if (!$array instanceof ArrayAccess && !is_array($array)) {
            throw new InvalidArgumentException(sprintf('Index "%s" cannot be modified in object of type "%s" because it doesn\'t implement \ArrayAccess', $index, get_class($array)));
        }
    ...
    private function writeProperty(&$object, $property, $singular, $value)
    {
        ...
        if (!is_object($object)) {
            throw new InvalidArgumentException(sprintf('Cannot write property "%s" to an array. Maybe you should write the property path as "[%s]" instead?', $property, $property));
        }
        ...
        $singulars = array($singular); //null !== $singular ? array($singular) : (array) StringUtil::singularify($plural);
        ...
        if ($reflClass->hasMethod($setter) && $reflClass->getMethod($setter)->isPublic()) {
            ...
            throw new InvalidArgumentException(sprintf(
                    ...
                    '"__set()" or "__call()" exist and have public access in class "%s".', $property, $guessedAdders, $setter, $reflClass->name
            ));
        }
    }
    ...
    {
        return preg_replace_callback('/(^|_|\.)+(.)/', array($this, 'preg_camelize'), $string);
    }
    ...
    private function findAdderAndRemover(ReflectionClass $reflClass, array $singulars)
    {
        foreach ($singulars as $singular) {
            ...
            if ($addMethodFound xor $removeMethodFound) {
                throw new InvalidArgumentException(sprintf(
                        'Found the public method "%s()", but did not find a public "%s()" on class %s', $addMethodFound ? $addMethod : $removeMethod, $addMethodFound ? $removeMethod : $addMethod, $reflClass->name
                ));
            }
        }
    ...
    public function preg_camelize($match)
    {
