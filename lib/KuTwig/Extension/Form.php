...
class KuTwig_Extension_Form extends Twig_Extension
{
    ...
    {
        return array(
            new Twig_SimpleFilter('choices', array($this, 'choices'), array('is_safe' => array('html'))),
        );
    }
     ...
     * @return type
     */
    public function choices($choices, $separator = ' ')
    {
     ...
     * @return mixed
     */
    private function propertyOrArrayValue($data, $index, $default = null)
    {
        ...
        if (is_object($data)) { // objeto
            ...
            //object method
            if (!isset(self::$twigCache[$class]['methods'])) {
                self::$twigCache[$class]['methods'] = array_change_key_case(array_flip(get_class_methods($data)));
            }

            $lcItem = strtolower($index);
